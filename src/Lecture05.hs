...
yearGDP now percent = iterate (\x -> x * (100 + percent) / 100) now
...
stat events = map (sumForCountry events) allCountries

sumForCountry :: [Country] -> Country -> Country
sumForCountry events country@(Country cName cCount) = Country cName (foldr (+) cCount (map (\(Country _ x) -> x) (filterEvents events country)))

filterEvents :: [Country] -> Country -> [Country]
filterEvents events (Country cName cCount) = filter (\(Country name _) -> name == cName) events
